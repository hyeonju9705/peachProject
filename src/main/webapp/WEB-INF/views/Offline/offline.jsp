...
<div id="header">
    ...
    <c:if test="${user.type eq 'b'}">
    	<button type="button" onclick="location.href='/Offline/store_add_off' ">내 상점등록</button>
	</c:if>
    <!--버튼끝-->
      ...
      <div class="album py-5 bg-light">
          ...
          <c:forEach items="${list}" var="list">
              ...
              <div class="card mb-4 box-shadow">
                ...
                <div class="card-body">
                  <p class="card-text">작성자 : ${list.writer} <br> 번호 : ${list.phone} <br> 주소 : ${list.addr} </p>
...
<div>
			<c:if test="${page.prev}">
 				<span>[ <a href="/Offline/offline?num=${page.startPageNum - 1}${page.searchTypeKeyword}">이전</a> ]</span>
			</c:if>
			...
			<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
 				<span>
  					...
  					<c:if test="${select != num}">
   						<a href="/Offline/offline?num=${num}${page.searchTypeKeyword}">${num}</a>
  					</c:if>    
			...
			<c:if test="${page.next}">
 				<span>[ <a href="/Offline/offline?num=${page.endPageNum + 1}${page.searchTypeKeyword}">다음</a> ]</span>
			</c:if>
		</div>
<div>
		...
		<script>
 			document.getElementById("searchBtn").onclick = function () {
 				...
 				location.href = "/Offline/offline?num=1" + "&searchType=" + searchType + "&keyword=" + keyword;
 			};
		</script>
